...
void txts_init(vector<DevicePart>& txt_container, vector<DevicePart>& txt_nbs_container) {
    ...
    DevicePart error_e = DevicePart(DEV_NB_W, DEV_NB_H);
    DevicePart error_r = DevicePart(DEV_NB_W, DEV_NB_H);

    txt_container = {controller_frame, controller_knob, device_frame, device_slider, device_light_red,
        device_light_green};
    txt_nbs_container = {number_0, number_1, number_2, number_3, number_4, number_5, 
        number_6, number_7, number_8, number_9, error_e, error_r};
}
